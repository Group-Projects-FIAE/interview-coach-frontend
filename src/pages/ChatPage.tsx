...
const ChatPage = () => {
  ...
  const handleSendUrl = () => {
    if (showInput) {
      // Clear the form on second click
      setInputText({ firstInput: "", secondInput: "" });
      setShowInput(false);
      return;
    }
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!inputText.firstInput.trim()) return;

    addMessage(inputText.firstInput, true);
    setInputText({ firstInput: "", secondInput: "" });
    setIsProcessing(true);

    await fetchAIResponse(inputText.firstInput);
  };
      ...
      <div className="chat-area">
        ...
        <form className="input-area" onSubmit={handleSendMessage}>
          ...
          <IconButton
            style={{ color: "white" }}
            type="submit"
            disabled={!inputText.firstInput.trim() || isProcessing}
